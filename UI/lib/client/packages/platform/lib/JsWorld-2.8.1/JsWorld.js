...
jsworld.parseIsoDateTime = function(isoDateTimeVal) {
	...
	// Note: the original ISO time spec for leap hours (24:00:00) and seconds (00:00:60) is not supported
	if (month < 1 || month > 12 ||
	    day   < 1 || day   > 31 ||
	    hour  < 0 || hour  > 23 ||
	    mins  < 0 || mins  > 59 ||
...
jsworld._hasOption = function(option, optionsString) {
	...
	return optionsString.indexOf(option) != -1;
};
 ...
 * @private
 *
 * @description Tests if a string starts with the specified substring. Matching
 * is conservative and case-insensitive: first the function attempts to do case
 * sensitive matching, to accommodate locales which don't have the notion of
 * case and where converting to lower/uppercase may be problematic; on failing
 * that it finally attempts a second match by converting both string to lower
 * case.
 *
...
jsworld._stringStartsWith = function (testedString, sub) {
	...
	var caseSensitiveMatchFailed = true;
	
	for (var i = 0; i < sub.length; i++) {
		if (testedString.charAt(i) != sub.charAt(i))
			caseSensitiveMatchFailed = true;
	}
	
	if (caseSensitiveMatchFailed) {
	
		// attempt second, case-insensitive matching
		for (var i = 0; i < sub.length; i++) {
			if (testedString.charAt(i).toLowerCase() != sub.charAt(i).toLowerCase())
				return false;
		}
	}
 ...
 * @param {String} optionsString The options string.
 *
 * @returns {Number} The requested precision, -1 if not specified.
 */
 ...
 *
 * @param {Number} number The number value subjected to selective padding.
 * @param {Number} length If the number has fewer digits than this length apply
 *        padding.
 *
 ...
 *
 *
 *        @param {Number} properties.frac_digits
 *
 ...
 *
 *
 *        @param {Number} properties.int_frac_digits
 *
 *        An integer representing the number of fractional digits (those to
 *        the right of the decimal delimiter) to be written in a formatted
 ...
 *
 *
 *        @param {Number} properties.p_cs_precedes
 *
 ...
 *
 *
 *        @param {Number} properties.n_cs_precedes
 *
 *        An integer set to 1 if the currency_symbol precedes the value for a
 ...
 *
 *
 *        @param {Number} properties.p_sep_by_space
 *
 ...
 *
 *
 *        @param {Number} properties.n_sep_by_space
 *
 *        Set to a value indicating the separation of the currency_symbol,
 ...
 *
 *
 *        @param {Number} properties.p_sign_posn
 *
 ...
 *
 *
 *	  @param {Number} properties.n_sign_posn
 *
 *	  An integer set to a value indicating the positioning of the
 ...
 *
 *
 *	  @param {Number} properties.int_p_cs_precedes
 *
 ...
 *
 *
 *	  @param {Number} properties.int_n_cs_precedes
 *
 *	  An integer set to 1 if the int_curr_symbol precedes the value for a
 *	  monetary quantity with a negative value, and set to 0 if the symbol
 *	  succeeds the value.
 *
 *
 *	  @param {Number} properties.int_p_sep_by_space
 *
 ...
 *
 *
 *	  @param {Number} properties.int_n_sep_by_space
 *
 *	  Set to a value indicating the separation of the int_curr_symbol,
 ...
 *
 *
 *	  @param {Number} properties.int_p_sign_posn
 *
 *	  An integer set to a value indicating the positioning of the
 ...
 *
 *
 *	  @param {Number} properties.int_n_sign_posn
 *
 *	  An integer set to a value indicating the positioning of the
...
jsworld.Locale = function(properties) {
	 ...
	 *
	 * @param {String[] | String} names An array or string defining the week /
     *        month names.
	 * @param {Number} expectedItems The number of expected list items, e.g. 7
     *        for weekdays, 12 for months.
	 *
	 ...
	 *
	 * @param {Number} [weekdayNum] An integer between 0 and 6. Zero
	 ...
	 *
	 * @param {Number} [monthNum] An integer between 0 and 11. Zero
	...
	this.intCurrencySymbolPrecedes = function() {
		...
		return this.int_p_cs_precedes == 1;
...
jsworld.NumericFormatter = function(locale) {
	...
	this.format = function(number, options) {
		...
		var formattedAmount =
			formattedFractionPart.length > 0 ?
...
jsworld.DateTimeFormatter = function(locale) {
	 ...
	 * 
	 * @returns {Number} Corresponding hour [1..12].
	 */
...
jsworld.MonetaryFormatter = function(locale, currencyCode, altIntSymbol) {
	...
	this.currencySymbolPrecedes = function(intFlag) {
		...
		if (typeof intFlag == "string" && intFlag == "i") {
			...
			return this.lc.int_p_cs_precedes == 1;
			...
			if (this.internationalFormatting) {
				...
				return this.lc.p_cs_precedes == 1;
			}
		}
	};
	...
	this.format = function(amount, options) {
		...
		if (formattedFractionPart.length > 0) {
		...
		while (fmtSpec.length > 0) {
			...
			if (fmtSpec.charAt(0) == "%" && fmtSpec.charAt(1) != "") {
				...
				if (placeholder == "%%") {
					...
					dt.day = parseInt(day[1], 10);
					
					if (isNaN(dt.day) || dt.day < 1 || dt.day > 31)
						throw "Parse error: Unrecognised day of the month (%e)";
					
					s = s.substring(day[0].length);
...
jsworld.MonetaryParser = function(locale) {
	...
	this._detectCurrencySymbolType = function(formattedCurrency) {
		...
		// Check for whichever sign (int/local) is longer first to cover cases
		// such as MOP/MOP$ and ZAR/R
	...
	this._normaliseNegativeSign = function(s, formatType) {
		...
		// check for enclosing parentheses rule and replace them with negative
