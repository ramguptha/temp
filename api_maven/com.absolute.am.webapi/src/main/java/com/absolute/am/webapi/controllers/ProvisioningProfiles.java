...
/**
 * <h3>Provisioning Profiles API</h3>
 * <p>This API is used to query Provisioning Profiles.</p>
 * 
 * @author gefimov
 *
 */
@Path ("/provisioningprofiles")
public class ProvisioningProfiles {
	
	private static Logger m_logger = LoggerFactory.getLogger(ProvisioningProfiles.class.getName());
    
    private static final String m_Base = ResourceUtilities.WEBAPI_BASE;

	private static final String VH_VIEW_NAME_ALL_PROVISIONING_PROFILES = "allprovisioningprofiles";
	private static final ViewDescription[] m_viewDescriptions = new ViewDescription[] {
		new ViewDescription(ViewHelper.VH_VIEW_NAME_ALL, "All Provisioning Profiles", 0),
	};
	...
	public ViewDescriptionList getViewsForProvisioningProfiles() throws Exception  {
		m_logger.debug("ProvisioningProfiles.getViewsForProvisioningProfiles called");
	 ...
	 *    For the "All" view columns are: Mobile Profile Name, Mobile Profile Description, Mobile Profile Organization, Mobile Platform Type,
	 *    Mobile Profile Type, Mobile Profile Identifier, Mobile Profile UUID,Mobile Profile Allow Removal and Mobile Profile Variables Used.</p>
	 * 
	 * <p>Rights required:</br>
	 *    None </p>
	 *    
	...
	public Result getView(
		...
		m_logger.debug("ProvisioningProfiles.getView called.");
		...
		if(viewname.compareToIgnoreCase(ViewHelper.VH_VIEW_NAME_ALL) == 0) {
			viewname = VH_VIEW_NAME_ALL_PROVISIONING_PROFILES;
		}
